...
const getSections = async (ID) => {
  try {
    ...
    const section = `${name_of_firm} ИИН/БИН ${zakazy_obwee[0].get(
      ...
      'тел2 (from клиент)'
    )} действующего в лице директора ${zakazy_obwee[0].get(
      'директор (from клиент)'
    )} далее именуемый <strong>«Заказчик»</strong>, с одной стороны, и ${
      ip[0]
    }, в лице директора ${rukovotidel}, именуемый в дальнейшем «Исполнитель», действующей на основании ${talon}, с другой стороны, совместно или по отдельности именуемые «Стороны», заключили настоящий договор (далее – Договор) о нижеследующем.`;
    ...
    const sections3 = `3.1. Общая стоимость настоящего Договора составляет ${totalAmount} (${numberToWordsRU(
      ...
      seventyPercent
    )})  тенге, 30% в размере ${thirtyPercent}  (${numberToWordsRU(
      thirtyPercent
